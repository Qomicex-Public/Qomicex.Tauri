...
namespace Qomicex.Core.Modules.Helpers.MultiPlatforms
{
    public class SystemMemoryHelper
    {
        /// <summary>
        /// 获取系统可用内存的指定百分比（单位：MB）
        /// </summary>
        /// <param name="percentage">使用比例，默认 0.6</param>
        /// <returns>MB，失败返回 -1</returns>
        ...
        public static long GetTotalPhysicalMemory()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return (long)GetTotalMemoryWindows() / (1024 * 1024); // 转换为 MB 
        ...
        /// <summary>
        /// Retrieves the amount of available physical memory on the current system, in bytes.
        /// </summary>
        /// <remarks>This method supports Windows, Linux, and macOS platforms. On unsupported platforms,
        /// the method returns -1. The returned value may fluctuate rapidly as system memory usage changes.</remarks>
        /// <returns>The number of bytes of available physical memory. Returns -1 if the operating system is not supported or the
        /// value cannot be determined.</returns>
        ...
        {
            // /proc/meminfo 中的 MemAvailable 是最靠谱的
            try
            {
            ...
            {
                Trace.WriteLine($"读取 /proc/meminfo 失败: {ex}");
            }
            return -1;
        }
        ...
        private static long GetAvailableMemoryMacOS()
        {
            ...
            {
                Trace.WriteLine($"获取 macOS 可用内存失败: {ex}");
                return -1;
            }
            finally
            {
        ...
        private static ulong GetTotalMemoryMac()
        {
            ...
            {
                Trace.WriteLine($"获取 macOS 总内存失败: {ex}");
            }
            finally
            {
