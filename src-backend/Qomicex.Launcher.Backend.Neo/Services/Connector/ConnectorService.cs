...
public sealed class ConnectorService : IDisposable
{
    ...
    private static string LauncherVersion
    {
        get
        {
            if (_cachedLauncherVersion != null) return _cachedLauncherVersion;
    ...
    private static string MachineId
    {
        get
        {
            ...
            var sysText = $"{GetOsName()}|{RuntimeInformation.OSDescription}|{Environment.OSVersion.VersionString}|{RuntimeInformation.OSArchitecture}";
    ...
    private async Task RunHostByInstanceAsync(GameInstance instance, CancellationToken ct)
    {
        try
        {
    ...
    public async Task LeaveAsync(CancellationToken ct = default)
    {
        ...
        {
            try { await _localHttp.PostAsync($"/api/instance/{_hostingInstanceId}/launch/cancel", null, ct); }
            catch { }
