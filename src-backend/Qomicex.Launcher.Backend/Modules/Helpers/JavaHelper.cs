...
namespace Qomicex.Launcher.Backend.Modules.Helpers
{
    public static class JavaHelper
    {
        ...
        private static readonly List<string> HighPriorityPaths = new()
        {
        ...
        private static List<JavaInfoExtended> SearchDeep(JavaSearchOptions options)
        {
            ...
            Parallel.ForEach(drives, new ParallelOptions { MaxDegreeOfParallelism = 4 }, drive =>
            {
                ...
                {
                    Debug.WriteLine($"扫描驱动器 {drive} 失败: {ex.Message}");
                }
            });
            ...
            HashSet<string> excludes)
        {
            ...
            while (queue.Count > 0 && results.Count < options.MaxResults)
            {
                ...
                {
                    Debug.WriteLine($"BFS 扫描 {currentPath} 失败: {ex.Message}");
                }
            }
        }
            ...
            JavaSearchOptions options)
        {
            ...
            foreach (var keyPath in registryKeys)
            {
                ...
                {
                    Debug.WriteLine($"读取注册表 {keyPath} 失败: {ex.Message}");
                }
            }
        }
            ...
            JavaSearchOptions options)
        {
            ...
            Parallel.ForEach(pathsToSearch, new ParallelOptions { MaxDegreeOfParallelism = 4 }, basePath =>
            {
                ...
                {
                    Debug.WriteLine($"扫描高优先级路径 {basePath} 失败: {ex.Message}");
                }
            });
        }
            ...
            JavaSearchOptions options)
        {
            ...
            try
            {
                Parallel.ForEach(Directory.GetDirectories(runtimePath), new ParallelOptions { MaxDegreeOfParallelism = 4 }, platformDir =>
                {
                    ...
                    {
                        Debug.WriteLine($"扫描 Minecraft runtime {platformDir} 失败: {ex.Message}");
                    }
                });
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"扫描 Minecraft runtime 失败: {ex.Message}");
            }
        }
            ...
            JavaSearchOptions options)
        {
            ...
            Parallel.ForEach(paths, new ParallelOptions { MaxDegreeOfParallelism = 4 }, pathEntry =>
            {
                ...
                {
                    Debug.WriteLine($"扫描 PATH 条目 {pathEntry} 失败: {ex.Message}");
                }
            });
        }
            ...
            string discoveredBy)
        {
            ...
            {
                Debug.WriteLine($"添加 Java {javaPath} 失败: {ex.Message}");
            }
        }

        public static JavaInfoExtended? ValidatePath(string path)
        {
            return GetJavaInfo(path, "Manual");
        }
        ...
        private static void TryGetVersionFromCommand(JavaInfoExtended javaInfo)
        {
            ...
            {
                Debug.WriteLine($"通过命令获取 Java 版本失败: {ex.Message}");
            }
        }
