...
- **Plugin API bridge**: `window.__PLUGIN_API__` (inline) / `parent.postMessage` (iframe) — methods: `getSettings`, `setSettings`, `callBackend`, `navigate`, `showToast`, `proxyFetchStream`, `registerMethod`, `callPlugin`, `readFile`, `writeFile`, `execCommand`, `overlay.*`. `proxyFetch` 走 `POST /api/plugins/proxy`（`stream: true` 时后端转发 SSE 流式响应，前端经 `proxyFetchStream(req, { onChunk, onError })` 消费）。文件读写走授权制（`/api/plugins/files/{id}/read|write|authorize`，未授权返回 403 `FS_AUTHORIZATION_REQUIRED`，前端 `window.confirm` 弹窗授权后重试）；shell 执行走 `/api/plugins/shell/{id}`（win: powershell，linux/mac: /bin/sh，超时默认 15s 范围 1-120s）。
