...
- 踢人（`/connector/kick`，仅房主）：`ScaffoldingCenter::kick_player` 断开流程——①解析 guest 的 easytier peer 并 `disconnect_peer`（优先已上报 `easytier_id`，否则按 hostname `scaffolding-mc-guest-{machine_id前8位}` 或 SCF TCP 源虚拟 IP 反查 `get_nodes()`；fork `EasyTier4QML` rev 8fccc86 新增的 `PeerManager::close_peer`/`Easytier::disconnect_peer` 全断连接 API；非 QML SCF 客户端不受 `qml:player_leave` 协议控制，仅此手段）；②记入已踢黑名单——被踢 guest 重连进入**审核状态机**（防误踢）：首次 re-ping 置 `pending`（`/connector/status` 响应 `pendingKickReviews[]`，前端弹窗三选经 `POST /connector/kick/review`：`allow` 移出黑名单 / `reject` 维持踢出 / `reject_silent` 置 `prompt_disabled` 后续静默 255），pending 期间响应状态 0 保持 SCF TCP 连接（玩家不入列）且持续断开 easytier；③`ClientRegistry::disconnect_machine` 按 machine_id 定向断开 Scaffolding TCP；④`remove_player`。**已知限制**：fork 无 peer 级控制面 deny，被踢 guest 的 easytier 连接仍会自动重连（ACL 仅数据面），黑名单仅在 SCF 层拦截其重新入列与数据面使用（重复断开 peer）；彻底封禁需给 fork 加控制面 deny。
