...
- **connector 架构定位（重要）**：`qomicex-connector-rust` 是**符合标准 SCF 协议的联机库**，只提供协议与拓展接口，**不内置业务功能**（踢人/黑名单/审核等一律由调用方实现）。拓展接口：`ScaffoldingCenter::set_player_ping_handler`（`c:player_ping` 裁决钩子，`create_room` 可选参数传入；返回 false → 状态 255 不刷新心跳，入列与否由调用方闭包决定）+ `handle_player_ping`（标准入列）+ 能力方法（`disconnect_machine` / `machine_source_ip` / `easy_tier_nodes` / `disconnect_peer` / `remove_player` / `get_players`）。改 connector 前先问：这是协议/接口还是业务功能？后者应放 backend。
- 踢人（`/connector/kick`，仅房主）：**实现位于调用方 `src-backend/qomicex-backend/src/services/kick.rs` `KickManager`**（经 connector 拓展接口组合实现，connector 零业务代码）——①解析 guest 的 easytier peer 并 `disconnect_peer`（优先已上报 `easytier_id`，否则按 hostname `scaffolding-mc-guest-{machine_id前8位}` 或 SCF TCP 源虚拟 IP 反查 `easy_tier_nodes()`；fork `EasyTier4QML` rev 8fccc86 的 `PeerManager::close_peer`/`Easytier::disconnect_peer` 全断连接 API；非 QML SCF 客户端不受 `qml:player_leave` 协议控制，仅此手段）；②记入已踢黑名单——被踢 guest 重连进入**审核状态机**（防误踢）：首次 re-ping 置 `pending`（`/connector/status` 响应 `pendingKickReviews[]`，前端弹窗三选经 `POST /connector/kick/review`：`allow` 移出黑名单 / `reject` 维持踢出 / `reject_silent` 置 `prompt_disabled` 后续静默 255），pending 期间响应状态 0 保持 SCF TCP 连接（玩家不入列）且持续断开 easytier；③`disconnect_machine` 按 machine_id 定向断开 Scaffolding TCP；④`remove_player`。**已知限制**：fork 无 peer 级控制面 deny，被踢 guest 的 easytier 连接仍会自动重连（ACL 仅数据面），黑名单仅在 SCF 层拦截其重新入列与数据面使用（重复断开 peer）；彻底封禁需给 fork 加控制面 deny。
