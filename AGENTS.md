...
- 踢人（`/connector/kick`，仅房主）：`ScaffoldingCenter::kick_player` 三层断开——①easytier `disconnect_peer`（fork `EasyTier4QML` rev 8fccc86 新增的 `PeerManager::close_peer`/`Easytier::disconnect_peer` 全断连接 API；非 QML SCF 客户端不受 `qml:player_leave` 协议控制，仅此手段）；②`ClientRegistry::disconnect_machine` 按 machine_id 定向断开 Scaffolding TCP（QML guest 心跳失败后整体退出）；③`remove_player`。**已知限制**：fork 无 peer 级控制面 deny，非 QML 客户端被踢后若持续在线可能自动重连（ACL 仅数据面）。
