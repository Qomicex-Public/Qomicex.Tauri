...
> 文档按分类目录存放，索引与 `docs/junsi-dev-docs/` 实际文件保持一致。
> 说明：ADR 编号存在历史空洞与冲突——`ADR-003`、`ADR-010` 无对应文件（编号空缺）；`ADR-015` 存有两个不同主题（NAT 检测 / 版权协议），为历史编号重复，保留原文件未改名。

## 1-决策记录

*ADR 架构决策记录*

- [ADR-001：Qomicex.Downloader → Qomicex.Downloader.Refactor 迁移](1-决策记录/ADR-001-Downloader-迁移.md)
...
- [ADR-016：启动器 I18N 国际化支持](1-决策记录/ADR-016-启动器I18N国际化支持.md)
...
- [ADR-020：日志体系完善：关请求噪音、简洁格式、业务日志持续落盘 + 查看器](1-决策记录/ADR-020-日志体系完善-关请求噪音-简洁格式-业务日志-持续落盘-查看器.md)
- [ADR-021：整合包本地导入与实例导出](1-决策记录/ADR-021-整合包本地导入与实例导出.md)
...
- [VERIFICATION LOG：联机房主解析复测](1-决策记录/VERIFICATION_LOG-联机房主解析复测.md)
- [Checkpoint 1：ServerManager 工厂方法（迁移）](1-决策记录/checkpoints/CHECKPOINT_BATCH_1.md)
- [Checkpoint 2：迁移批次记录](1-决策记录/checkpoints/CHECKPOINT_BATCH_2.md)
...
- [联机故障诊断：easytier 中继数据面](2-架构设计/联机故障诊断-easytier中继数据面.md)
- [联机故障诊断：easytier 中继数据面未打通](2-架构设计/联机故障诊断-easytier中继数据面未打通.md)

## 3-API规范

*RESTful API 设计规范与协议说明*

- [API 端点参考](3-API规范/API列表.md)
- [插件系统 API](3-API规范/插件系统API.md)
- [SCF 联机扩展协议规范](3-API规范/SCF联机扩展协议规范.md)
...
> UI 分区另有 [分区索引与组件清单](6-UI/组件设计/README.md)。
...
| 日期 | 变更 |
|---|---|
| 2026-08-18 | **索引刷新**：补齐此前遗漏的 ADR-016~024 与 I18N 国际化设计、SCF 联机扩展协议规范等条目；修正 ADR 编号空洞/冲突说明（ADR-003/010 无文件，ADR-015 存在两个主题）。**新增 ADR-025**：出站 HTTP UA 统一 + 目录管理默认「当前目录」占位入口。**迁移留档归位**：根目录散落的 `CHECKPOINT_BATCH_1/2.md` 移入 `1-决策记录/checkpoints/`，`VERIFICATION_LOG.md` 移入 `1-决策记录/`，并补提交此前未跟踪的 ADR-023/024/025。 |
