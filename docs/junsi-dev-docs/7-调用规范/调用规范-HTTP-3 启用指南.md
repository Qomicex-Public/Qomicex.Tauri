...
> 2026-08-19：曾试过 HTTP/1.1-only 提速，实测更慢（450KB/s vs 800KB/s），已**回滚保留 HTTP/2**。若需国内提速，正路是切**下载源为 BMCLAPI 镜像**（国内快，20MB/s+），而非改协议。
> 2026-08-19（提速关键）：大量小文件场景的瓶颈是**逐文件 HEAD 探测**。已加**按宿主缓存的探测缓存**（同一 CDN 主机第一个文件才探测，其余直接 GET，向 Java 启动器对齐；任务失败时清缓存保留重试语义）。同主机几千个小文件下载从 1-3s/文件 降到直连 GET 量级。
